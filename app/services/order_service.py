...
from app.schemas.order import Direction
...
class OrderService:
    ...
    async def create_order(
        ...
        if order_data.direction == Direction.BUY:
            ...
            if not is_limit_order:
                raise NotImplementedError("Balance check for market BUY orders is not yet fully implemented without price estimation.")
        ...
        if not blocked_successfully:
            raise ValueError(f"Insufficient balance of {ticker_to_block_or_check} to place order.")
        ...
        try:
            ...
            if not created_order_row:
                logger.error(f"Order creation failed for user {user_id_obj} after funds were (supposedly) blocked.")
            ...
            logger.error(f"Error creating order for user {user_id_obj} (service level): {e}", exc_info=True)
            raise
    ...
    async def cancel_order(
        ...
        if current_status == OrderStatus.NEW or current_status == OrderStatus.PARTIALLY_EXECUTED:
            amount_to_unblock = 0
            ticker_to_unblock = ""

            remaining_qty_to_unblock = order_to_cancel_map["qty"] - order_to_cancel_map["filled_qty"]
            ...
            if remaining_qty_to_unblock > 0:
                if order_to_cancel_map["direction"] == Direction.BUY:
                    ...
                    if order_to_cancel_map["price"] is None:
                        logger.warning(f"Cannot determine amount to unblock for market BUY order {order_id} without price.")
                    else:
                        amount_to_unblock = order_to_cancel_map["price"] * remaining_qty_to_unblock
                ...
                if ticker_to_unblock and amount_to_unblock > 0:
                    ...
                    if not unblocked_successfully:
                        raise Exception(f"Failed to unblock funds for order {order_id}, cancelling operation.")
                else:
        ...
        if unblocked_successfully:
            ...
            if updated_id:
                logger.info(f"Order {order_id} successfully cancelled and funds (if any) unblocked by user {user_id}.")
                return True
            else:
                logger.error(f"Failed to update status for order {order_id} after funds unblocked.")
                raise Exception("Failed to update order status after unblocking funds.")
        else:
            logger.error(f"Fund unblocking failed for order {order_id}, cancellation aborted.")
