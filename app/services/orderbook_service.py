...
class OrderBookService:
    ...
    async def get_orderbook(self, ticker: str, limit: int = 10) -> Optional[L2OrderBook]:
        ...
        try:
            ...
            instrument_exists_stmt = select(func.count(instruments_table.c.id))\
            ...
            bids_stmt = select(
                ...
                func.sum(remaining_qty).label("total_qty")
            ).where(
                ...
                desc(orders_table.c.price) # Лучшие цены покупки (самые высокие) идут первыми
            ).limit(limit)
            ...
            bid_levels = [Level(price=row['price'], qty=row['total_qty']) for row in bid_rows if row['price'] is not None]
            ask_levels = [Level(price=row['price'], qty=row['total_qty']) for row in ask_rows if row['price'] is not None]
