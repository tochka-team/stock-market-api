...
class OrderBookService:
    ...
    async def get_orderbook(self, ticker: str, limit: int = 10) -> Optional[L2OrderBook]:
        ...
        try:
            ...
            bid_levels = [Level(price=row['price'], qty=row['total_qty'])
                          for row in bid_rows if row['price'] is not None]
            ask_levels = [Level(price=row['price'], qty=row['total_qty'])
                          for row in ask_rows if row['price'] is not None]
