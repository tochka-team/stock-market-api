...
async def get_logs_endpoint(
    ...
    end_time: Optional[str] = Query(
        None, description="Конечное время в ISO формате (например: 2024-01-01T23:59:59)"
    ),
    ...
    offset: Optional[int] = Query(0, description="Смещение для пагинации", ge=0),
):
    """
    ...
    try:
        ...
        return LogsResponse(logs=logs, total_count=total_count, has_more=has_more)
...
async def get_recent_logs_endpoint(
    lines: Optional[int] = Query(50, description="Количество последних строк", ge=1),
    level: Optional[str] = Query(
        None, description="Уровень логирования (DEBUG, INFO, WARNING, ERROR, CRITICAL)"
    ),
):
    ...
    try:
        ...
        return LogsResponse(logs=logs, total_count=total_count, has_more=False)
